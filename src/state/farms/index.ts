...
export const fetchLaboPriceAsync = () => async (dispatch) => { // Change the address below to LABO-BUSD LP
  ...
  {ethereum(network: bsc){
    address(address: {is: "${labo.addr.laboBnbAddr}"}){
    balances {
    currency {
    symbol
    }
...
fetch(url, opts)
  ...
  .then(json => {
    ...
    const lprice = json.data.ethereum.address[0].balances[Labo.queryPosition.bnb].value / json.data.ethereum.address[0].balances[labo.queryPosition.token].value;
